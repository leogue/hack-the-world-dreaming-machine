...
def build_ssl(encoder, cfg):
    ...
    class EEGPredictiveVICRegJEPA(nn.Module):
        ...
        def compute_loss(self, batch):
            # SSL loader returns two augmented EEG views: (v1, v2)
            ...
            F_total = online_frames.shape[1]

            split_idx = int(0.8 * F_total)

            context = online_frames[:, :split_idx]
            target  = target_frames[:, split_idx:]

            target_len = target.shape[1]

            _, h = self.predictor(context)
            h = h[-1]
            ...
            # EB-JEPA energy = prediction error in representation space
            pred_loss = F.mse_loss(pred, target.detach())

            # VICReg-style anti-collapse regularization on projected online latents
            z = online_frames.reshape(-1, online_frames.shape[-1])
            z = self.projector(z)

            reg_out = self.regularizer(z)
            reg_loss, reg_logs = self._parse_reg_loss(reg_out, x_context.device)
        ...
        def compute_energy(self, batch):
            ...
            energy_per_step = ((pred - target) ** 2).mean(dim=-1)
            energy_per_window = energy_per_step.mean(dim=1)
