...
\textbf{Repo + venv.} Work inside the shared project space \code{/lustre/work/vivatech-<team>/}
...
\begin{lstlisting}[style=bash]
cd /lustre/work/vivatech-<team>/<you>/eb_jepa
source env.sh    # -> venvs/eb_jepa_{x86_64|aarch64}; then `uv pip install ...` as needed
\end{lstlisting}

\textbf{SLURM.} Submit with \code{--account=vivatech-<team> --partition=defq --gres=gpu:1};
...
hackathon cluster use \code{--account=vivatech-<team> --partition=defq} (and
...
\smallskip
\textbf{poc:} the minimal Two Rooms recipe is zero-shot \emph{robust} to appearance
(dot blur: \textbf{0} degradation) and wall thickness, but planning \emph{collapses as the
door narrows}: \textbf{100\%$\to$40\%$\to$0\%} success at door half-gap $4\to2\to1$ (3 seeds,
MPPI). Controls confirm a genuine world-model deficit, not task or perception hardness: a
true-state oracle solves \textbf{100\%} at every width (random \textbf{0\%}), and the failure
persists under $8\times$ planner budget and a dot shrunk to fit the gap. Mechanistically it
does \emph{not} track position-probe or rollout error (both worst for the \emph{benign} blur
factor) but co-occurs with an encoder representation rank collapse that VICReg's
per-dimension variance misses; correspondingly it is \emph{not} a loss-weighting knob ---
strengthening any regularizer term backfires (an inverted-U around the default). The one
lever that helps is encoder \emph{capacity} (door$=$2: \textbf{40\%$\to$78\%} as \code{dstc}
$32\to256$), yet even $8\times$ capacity never recovers the narrowest door --- the toy
recipe's fragility is a representational-capacity limit, not a tuning knob.
\emph{Data:} Two Rooms, generated online (no dataset path).
\end{trackbox}
...
light additions to the video eval (\code{examples/intuitive\_physics}).
\textbf{Metric:} the energy gap (impossible $>$ plausible) and how it grows over
training. Exploratory, cheap, and genuinely open at this scale.

\textbf{poc:} on procedural bouncing-MNIST, a video-JEPA's \code{predcost} energy
detects physically impossible clips at \textbf{AUROC $\approx$1.0} (vs 0.5 at init),
but controls show this is largely \emph{out-of-distribution motion}: a one-line
max-displacement baseline matches it on teleport / wall-passthrough, and it flags
\emph{physically legal} fast clips at \textbf{1.0}. The genuine, magnitude-matched
signal is an instant \emph{velocity reversal} (\textbf{0.999} vs \textbf{0.47}
trivial) and \emph{freeze} (0.99 vs 0.45), which require the latent-prediction
objective (a generative pixel model's own latent reaches only \textbf{0.5--0.8})
and anti-collapse regularization (none $\rightarrow$ rank-1 collapse $\rightarrow$
0.5). Answer to the open question: a sensitive OOD-motion detector with a real but
narrow learned-dynamics core, not general physics understanding.
\end{trackbox}
